		...
		<>
			<footer className="panel flex items-center justify-between px-4 h-7 text-xs border-t border-[var(--ps-border)]">
				...
				<div className="flex items-center gap-4 text-[var(--ps-text-muted)]">
					...
					<span>
						画像: {enabledImageCount}/{imageCount}
					</span>
				</div>
			</footer>
	...
	return (
		<div
			className={`w-2 h-2 rounded-full ${
				isProcessing
					? "bg-[var(--ps-warning)] animate-pulse"
					: "bg-[var(--ps-success)]"
			}`}
		/>
	);
...
function LocalLaunchModal({ onClose }: { onClose: () => void }) {
	...
	const commands = {
		...
		install:
			"cd Qwen-Image-Edit-2511 && npm install && pip install -r python/requirements.txt",
			...
			>
				<div className="flex items-center justify-between p-4 border-b border-[var(--ps-border)]">
					<h2 className="text-lg font-semibold flex items-center gap-2">
						<svg
							aria-hidden="true"
							className="w-5 h-5 text-[var(--ps-accent)]"
							fill="none"
							stroke="currentColor"
							viewBox="0 0 24 24"
						>
					...
					<button
						onClick={onClose}
						className="text-[var(--ps-text-muted)] hover:text-white"
					>
						<svg
							aria-hidden="true"
							className="w-5 h-5"
				...
				<div className="p-4 space-y-4">
						...
						<div className="space-y-2">
							<div className="bg-[var(--ps-bg-dark)] rounded p-2">
								<div className="flex items-center justify-between mb-1">
									<span className="text-xs text-[var(--ps-text-muted)]">
										リポジトリをクローン
									</span>
									<button
					...
					<div className="bg-[var(--ps-bg-light)] rounded p-3">
						...
						<div className="grid grid-cols-2 gap-2 text-sm text-[var(--ps-text-muted)]">
							<div>
								<span className="text-[var(--ps-text)]">GPU:</span> NVIDIA 8GB+
								VRAM
							</div>
							<div>
								<span className="text-[var(--ps-text)]">RAM:</span> 16GB+
							</div>
							<div>
								<span className="text-[var(--ps-text)]">Python:</span> 3.10+
							</div>
							<div>
								<span className="text-[var(--ps-text)]">CUDA:</span> 11.8+
							</div>
						</div>
					</div>
