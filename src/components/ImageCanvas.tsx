...
import type { GenerationStatus, ImageFile } from "../types";
					...
					<div className="grid grid-cols-4 gap-3 h-[calc(100%-24px)]">
						...
						{[0, 1, 2, 3].map((index) => (
							<ImageSlot
								...
								getRootProps={
									images.length === index ? getRootProps : undefined
								}
								getInputProps={
									images.length === index ? getInputProps : undefined
								}
...
function ImageSlot({
	...
	if (image) {
		return (
			<div
				className={`panel rounded-lg overflow-hidden relative group ${!image.enabled ? "opacity-40" : ""}`}
			>
				<img
					src={image.preview}
					alt={`Input ${index + 1}`}
