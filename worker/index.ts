...
import type { BackendStatus, Env } from "./types";
...
async function checkReplicateAPI(
	...
	try {
		...
		const res = await fetch(
			"https://api.replicate.com/v1/models/qwen/qwen-image-edit-2511",
			{
				headers: {
...
app.get("/api/models", async (c) => {
	try {
		...
		return c.json(
			{ error: true, message: "モデル一覧の取得に失敗しました" },
			500,
		);
	}
});
...
app.post("/api/generate", async (c) => {
	try {
		...
		const resolution = parseInt(
			(formData.get("resolution") as string) || "1024",
			10,
		);
		// modelId または model_id の両方をサポート
		const modelId =
			(formData.get("modelId") as string) ||
			(formData.get("model_id") as string) ||
			null;
		...
		if (activeModelId === "bagel-7b-mot") {
			...
			// Cloudflare Workersではローカルサーバーが必要
			return c.json(
				{
					error: true,
					message: "Real-ESRGAN超解像度はローカルサーバーでのみ利用可能です。ローカル環境で実行してください。",
				},
				503
			);
			...
			return c.json(
				{
					error: true,
					message: "FLUX.1 ONNXはローカルサーバーでのみ利用可能です。ローカル環境で実行するか、flux2-dev（クラウド版）をお使いください。",
				},
				503
			);
		} else if (activeModelId === "bagel-7b-mot-int8") {
			// BAGEL INT8量子化版（ローカルサーバー必須）
			return c.json(
				{
					error: true,
					message: "BAGEL INT8量子化版はローカルサーバーでのみ利用可能です。bagel-7b-mot（クラウド版）をお使いください。",
				},
				503
			);
			...
			if (backendStatusCache.mode === "local") {
				...
				if (modelType === "cloud" || activeModelId === "huggingface-cloud") {
					const hfUrl =
						selectedModel?.source || c.env.HF_SPACE_URL || DEFAULT_HF_SPACE_URL;
							...
							return c.json(
								{
									error: true,
									message:
										"Qwen-Image-Editは画像編集モデルです。編集する画像をアップロードしてください。",
								},
								400,
							);
						}
		...
		return c.json({
			...
			model: selectedModel
				...
				: { id: activeModelId, name: activeModelId },
			modelId: activeModelId,
		});
	} catch (error) {
...
async function callReplicateAPI(
	...
	if (!createRes.ok) {
		...
		try {
			...
			throw new Error(
				error.detail || `Replicate APIエラー (${createRes.status})`,
			);
		} catch (e) {
			if (e instanceof Error && e.message.includes("Replicate")) throw e;
			throw new Error(
				`Replicate APIでの生成に失敗しました (${createRes.status}): ${errorText.substring(0, 100)}`,
			);
		}
	}
	...
	if (prediction.urls?.get) {
		...
		while (attempts < maxAttempts) {
			...
			if (pollData.status === "failed") {
				throw new Error(
					pollData.error || "Replicate APIでの生成に失敗しました",
				);
			}
...
async function callReplicateStableDiffusion(
	...
	if (!createRes.ok) {
		...
		try {
			const error = JSON.parse(errorText) as { detail?: string };
			throw new Error(
				error.detail || `Stable Diffusion APIエラー (${createRes.status})`,
			);
		} catch (e) {
			if (e instanceof Error && e.message.includes("Stable Diffusion")) throw e;
			throw new Error(
				`Stable Diffusion APIでの生成に失敗しました (${createRes.status}): ${errorText.substring(0, 100)}`,
			);
		}
	}
	...
	if (prediction.urls?.get) {
		...
		while (attempts < maxAttempts) {
			...
			if (pollData.status === "failed") {
				throw new Error(
					pollData.error || "Stable Diffusion APIでの生成に失敗しました",
				);
			}
...
async function callGradioAPI(
	...
	const galleryImages: Array<{
		image: { url: string; meta: { _type: string } };
	}> = [];
	...
	if (text.includes("event: error")) {
		console.error(
			"Qwen Space returned error event - likely ZeroGPU quota exceeded",
		);
		throw new Error(
			"Qwen SpaceのZeroGPU割り当てが一時的に枯渇しています。しばらく待ってから再試行するか、Z-Image-Turboなど他のモデルをお試しください。",
		);
	}
	...
	for (const line of lines) {
		if (line.startsWith("data: ")) {
			try {
				...
				if (Array.isArray(data) && data.length > 0) {
					const galleryOutput = data[0] as Array<{
					...
					if (Array.isArray(galleryOutput) && galleryOutput.length > 0) {
						...
						if (imageUrl) {
							...
							const fullUrl = imageUrl.startsWith("http")
								? imageUrl
								: `${hfUrl}${imageUrl.startsWith("/") ? "" : "/"}${imageUrl}`;
	...
	throw new Error(
		"画像の生成に失敗しました。HuggingFace SpaceのZeroGPU制限により、クラウド生成が利用できない場合があります。Z-Image-Turboなど他のモデルを選択するか、ローカルPythonサーバーの使用を検討してください。",
	);
}
...
async function callBagelTextToImageAPI(prompt: string): Promise<string> {
	...
	const queueRes = await fetch(
		`${BAGEL_SPACE_URL}/gradio_api/call/text_to_image`,
		{
			method: "POST",
			headers: {
				"Content-Type": "application/json",
	...
	const streamRes = await fetch(
		`${BAGEL_SPACE_URL}/gradio_api/call/text_to_image/${eventId}`,
	);
...
async function callBagelEditAPI(prompt: string, image: File): Promise<string> {
	...
	// Gradio API呼び出し
	const queueRes = await fetch(
		`${BAGEL_SPACE_URL}/gradio_api/call/edit_image`,
		{
			method: "POST",
			headers: {
				"Content-Type": "application/json",
	...
	// イベントストリームから結果を取得
	const streamRes = await fetch(
		`${BAGEL_SPACE_URL}/gradio_api/call/edit_image/${eventId}`,
	);
...
async function callZImageAPI(prompt: string): Promise<string> {
	...
	// イベントストリームから結果を取得
	const streamRes = await fetch(
		`${ZIMAGE_SPACE_URL}/gradio_api/call/generate/${eventId}`,
	);
	...
	if (text.includes("event: error")) {
		...
		throw new Error(
			"Z-Image Space一時的にエラー状態です。後でお試しください。",
		);
	}
			...
			try {
				...
				if (Array.isArray(data) && data[0] && Array.isArray(data[0])) {
					...
					if (galleryData.length > 0) {
						...
						console.log(
							"Z-Image image info:",
							JSON.stringify(imageInfo).substring(0, 200),
						);
						...
						if (imageInfo?.url) {
							...
							if (imageInfo.path.startsWith("http")) {
								...
								if (!imageRes.ok) {
									console.error(
										"Z-Image image fetch from path failed:",
										imageRes.status,
									);
									throw new Error("画像の取得に失敗しました");
								}
								...
								if (!imageRes.ok) {
									console.error(
										"Z-Image image fetch from constructed URL failed:",
										imageRes.status,
									);
									throw new Error("画像の取得に失敗しました");
								}
...
async function callStableDiffusionHF(
	...
	if (!streamRes.ok) {
		console.error(
			"Stable Diffusion HF stream error, falling back to Z-Image-Turbo",
		);
		return await callZImageAPI(prompt);
	}
	...
	if (text.includes("event: error")) {
		console.error(
			"Stable Diffusion Space returned error, falling back to Z-Image-Turbo",
		);
		return await callZImageAPI(prompt);
	}
	...
	console.log(
		"Stable Diffusion HF parse failed, falling back to Z-Image-Turbo",
	);
	return await callZImageAPI(prompt);
}
...
async function callFlux2API(
	...
	// イベントストリームから結果を取得
	const streamRes = await fetch(
		`${FLUX2_SPACE_URL}/gradio_api/call/infer/${eventId}`,
	);
...
app.post("/api/admin/models", authMiddleware, async (c) => {
	try {
		...
		const { name, type, source, description, backends, isDefault, enabled } =
			body;
		...
		return c.json(
			{
				error: true,
				message:
					error instanceof Error ? error.message : "モデルの追加に失敗しました",
			},
			400,
		);
	}
		...
		return c.json(
			{
				error: true,
				message:
					error instanceof Error ? error.message : "モデルの更新に失敗しました",
			},
			400,
		);
	}
		...
		return c.json(
			{
				error: true,
				message:
					error instanceof Error ? error.message : "モデルの削除に失敗しました",
			},
			400,
		);
	}
